                ...
                for NS in num_stages_list:
                    ...
                    try:
                        ms = triton.testing.do_bench(
                            run_current_config, warmup=25, rep=100
                        )
...
def benchmark():
    ...
    results = []
    print(
        f"{'Config (B, H, N, D)':<25} | {'Provider':<20} | {'Time (ms)':<10} | {'TFLOPS':<10} | {'Speedup vs Torch (Def)':<20}"
    )
    print("-" * 100)
    ...
    for B, H, N, D in configs:
        ...
        with sdpa_kernel(SDPBackend.FLASH_ATTENTION):
            ms_torch_opt = triton.testing.do_bench(
                lambda: torch.nn.functional.scaled_dot_product_attention(
                    q_opt, k_opt, v_opt, is_causal=False, dropout_p=0.0
                )
            )
        ...
        print(
            f"{cfg_str:<25} | {'PyTorch (Default)':<20} | {ms_torch_def:<20.3f} | {tflops_torch_def:<10.2f} | 1.00x"
        )
        print(
            f"{'':<25} | {'PyTorch (Opt)':<20} | {ms_torch_opt:<20.3f} | {tflops_torch_opt:<10.2f} | {ms_torch_def/ms_torch_opt:.2f}x"
        )
        print(
            f"{'':<25} | {'Your Triton':<20} | {ms_triton:<20.3f} | {tflops_triton:<10.2f} | {ms_torch_def/ms_triton:.2f}x"
        )
        print("-" * 100)
